...
- "객실 보여줘/추천/가격/방 보여줘" 같은 요청이면 ROOM_LIST 또는 ROOM_CARDS로 객실을 보여주는 게 좋습니다. (기본 선호는 ROOM_LIST)
- "예약 내역/조회/취소"는 HISTORY로 응답하세요.
- 예약 확정은 CONFIRM 타입으로 진행합니다. (필수: roomId, checkIn, checkOut) 이름/전화번호는 받지 않습니다.
- 사용자가 "1박"이라 말해도 checkOut을 자동 계산하지 마세요. checkOut은 반드시 사용자에게 받아야 합니다.
- 체크인은 오늘 기준 30일 이내만 가능(서버 검증). 날짜 형식은 YYYY-MM-DD가 안정적입니다.
...
function renderRoomList(rooms, roomIds, caption) {
  ...
  // 각 객실 선택 버튼은 listCard items 자체엔 버튼이 없어서, 별도 캐러셀로 2~3개만 제공
  const carouselItems = picked.slice(0, 5).map(r => ({
    title: r.name,
    description: `${r.short || ""}`.trim() || "선택",
    thumbnail: r.images?.[0] ? { imageUrl: r.images[0] } : undefined,
    buttons: [
      { action: "message", label: "이 객실 선택", messageText: `CMD:SET_ROOM:${enc(r.id)}` },
      { action: "message", label: "이 객실로 예약", messageText: `CMD:SET_ROOM:${enc(r.id)}` }
    ]
  }));

  outputs.push(outCarouselBasicCard(carouselItems));
...
function renderHistory(rooms, bookings, caption) {
  ...
  const items = bookings.slice(0, 10).map(b => {
    ...
    const title = `${b.id} (${b.status})`;
    const description = [`객실: ${roomName}`, `체크인: ${b.checkIn}`, `체크아웃: ${b.checkOut}`].join("\n");
    return {
      title,
      description,
